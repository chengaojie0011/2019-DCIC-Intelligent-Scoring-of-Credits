...
from feature_engineering_wolf import Processing
...
def lgb_mse_model(train_df, test_df, params):
    ...
    for i, (train_fold, validate) in enumerate(kf):
        ...
        bst = lgb.train(params, dtrain, num_boost_round=10000, valid_sets=dvalid, verbose_eval=-1, early_stopping_rounds=2000)
        val_pred = pd.DataFrame()
        val_pred['id'] = val_id_target['id']
        val_pred['index'] = X_validate.index
        val_pred['target'] = label_validate
...
def xgb_model(train_df, test_df, params):
    ...
    for i, (train_fold, validate) in enumerate(kf):
        ...
        bst = gbm.fit(X_train, label_train, eval_set=[(X_train, label_train), (X_validate, label_validate)],
                    eval_metric ='mae',verbose=500,early_stopping_rounds=2000)
...
#    lgb_mae_val['score'] = lgb_mae_val['score'].apply(lambda x: int(np.round(x)))
    lgb_mae_val.to_csv('input/lgb_mae_w_1_train.csv', index=False)     
...
#    test_mae_lgb_sub1[['id', 'score']].to_csv('input/lgb_mae_xxx_kf10_testpush.csv', index=False)
    test_mae_lgb_sub1[['id', 'score']].to_csv('input/lgb_mae_w_1_test.csv', index=False)
...
#    lgb_mse_val['score'] = lgb_mse_val['score'].apply(lambda x: int(np.round(x)))
    lgb_mse_val.to_csv('input/lgb_mse_w_1_train.csv', index=False)    
...
#    test_mse_lgb_sub2[['id', 'score']].to_csv('input/lgb_mse_xxx_w_testpush.csv', index=False)    
    test_mse_lgb_sub2[['id', 'score']].to_csv('input/lgb_mse_w_1_test.csv', index=False)    
...
#    val_ctb_sub['score'] = val_ctb_sub['score'].apply(lambda x: int(np.round(x)))
    ctb_val.to_csv('input/ctb_w_1_train.csv', index=False)    
...
#    test_ctb_sub3[['id', 'score']].to_csv('input/ctb_xxxpush_w_test.csv', index=False)     
    test_ctb_sub3[['id', 'score']].to_csv('input/ctb_w_1_test.csv', index=False)       
