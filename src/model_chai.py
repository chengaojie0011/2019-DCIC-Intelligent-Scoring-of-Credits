...
from feature_engineering_chai import Processing
...
#        'random_state': 4590,
    ...
    return params_mae_lgb, params_mse_lgb, params_mae_xgb, params_ctb
...
def lgb_mse_model(train_df, test_df, params):
    ...
    for i, (train_fold, validate) in enumerate(kf):
        ...
        bst = lgb.train(params, dtrain, num_boost_round=10000, valid_sets=dvalid, verbose_eval=-1, early_stopping_rounds=1000)
        val_pred = pd.DataFrame()
        val_pred['id'] = val_id_target['id']
        val_pred['index'] = X_validate.index
        val_pred['target'] = label_validate
...
#    lgb_mae_val['score'] = lgb_mae_val['score'].apply(lambda x: int(np.round(x)))
    lgb_mae_val.to_csv('input/lgb_mae_c_1_train.csv', index=False)     
...
#    test_mae_lgb_sub1[['id', 'score']].to_csv('input/lgb_mae_xxx_kf10_testpush.csv', index=False)
    test_mae_lgb_sub1[['id', 'score']].to_csv('input/lgb_mae_c_1_test.csv', index=False)
...
#    lgb_mse_val['score'] = lgb_mse_val['score'].apply(lambda x: int(np.round(x)))
    lgb_mse_val.to_csv('input/lgb_mse_c_1_train.csv', index=False)    
...
#    test_mse_lgb_sub2[['id', 'score']].to_csv('input/lgb_mse_xxx_g_testpush.csv', index=False)    
    test_mse_lgb_sub2[['id', 'score']].to_csv('input/lgb_mse_c_1_test.csv', index=False)    
#
#    #xgb
##    train_data = get_feature(name="train_data_gavin")
##    test_data = get_feature(name="test_data_gavin")
...
#    xgb_val.to_csv('input/xgb_718_kf10_train.csv', index=False)    
#   
#    #输出xgb_test结果
...
#    test_xgb_sub3[['id', 'score']].to_csv('input/xgb_718_kf10_test.csv', index=False)   
#    
     #ctb
#    train_data = get_feature(name="train_data_gavin")
#    test_data = get_feature(name="test_data_gavin")
...
#    val_ctb_sub['score'] = val_ctb_sub['score'].apply(lambda x: int(np.round(x)))
    ctb_val.to_csv('input/ctb_c_1_train.csv', index=False)    
...
#    test_ctb_sub3[['id', 'score']].to_csv('input/ctb_xxxpush_c_test.csv', index=False)     
    test_ctb_sub3[['id', 'score']].to_csv('input/ctb_c_1_test.csv', index=False)     
