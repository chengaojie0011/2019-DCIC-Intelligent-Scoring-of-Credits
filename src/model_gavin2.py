...
def lgb_mae_model(train_df, test_df, params):
    ...
    kfold = KFold(n_splits=NFOLDS, shuffle=True, random_state=27)
    ...
    for i, (train_fold, validate) in enumerate(kf):
        ...
        bst = lgb.train(params, dtrain, num_boost_round=10000, valid_sets=dvalid,
                        verbose_eval=-1, early_stopping_rounds=500)        
...
#    lgb_mae_val['score'] = lgb_mae_val['score'].apply(lambda x: int(np.round(x)))
    lgb_mae_val.to_csv('input/lgb_mae_g_2_train.csv', index=False)     
...
#    test_mae_lgb_sub1[['id', 'score']].to_csv('input/lgb_mae_xxx_kf10_testpush.csv', index=False)
    test_mae_lgb_sub1[['id', 'score']].to_csv('input/lgb_mae_g_2_test.csv', index=False)
...
##    lgb_mse_val['score'] = lgb_mse_val['score'].apply(lambda x: int(np.round(x)))
    lgb_mse_val.to_csv('input/lgb_mse_g_2_train.csv', index=False)    
...
#    test_mse_lgb_sub2[['id', 'score']].to_csv('input/lgb_mse_2_g_testpush.csv', index=False)    
    test_mse_lgb_sub2[['id', 'score']].to_csv('input/lgb_mse_g_2_test.csv', index=False)    
...
#    val_ctb_sub['score'] = val_ctb_sub['score'].apply(lambda x: int(np.round(x)))
    ctb_val.to_csv('input/ctb_g_2_train.csv', index=False)    
...
#    test_ctb_sub3[['id', 'score']].to_csv('input/ctb_xxxpush_g_test.csv', index=False)     
    test_ctb_sub3[['id', 'score']].to_csv('input/ctb_g_2_test.csv', index=False)     
