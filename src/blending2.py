...
TRAIN_DATA_PATH = os.path.join(BASE_PATH, "blending2")
TEST_DATA_PATH = os.path.join(BASE_PATH, "blending2")
NFOLDS = 10
...
def get_blending(train_df, test_df):
    ...
    train_cp = model_weight(train_cp,'lgb_mae_c_1')
    valid = mean_absolute_error(y_true= train_df['target'] , y_pred= (train_cp['score']).apply(lambda x: int(math.ceil(x)))) 
    ...
    test_cp['score'] =0
    test_cp = model_weight(test_cp,'lgb_mae_c_1')   
    return test_cp
